# Talocode Roadmap

## Phase 1 — Foundation (Current)

Establish the brand, the hosted API layer, and the first wave of open-source tools.

- [x] Brand repository — this repo
- [x] talocode.site — landing page (GitHub Pages, legacy alias: talocode.xyz)
- [x] Stacklane billing — prepaid wallet, API keys, usage metering
- [x] Talocode Cloud router — OpenAI-compatible provider router with fallback
- [ ] Agent Browser API — hosted browser validation with Talocode Cloud billing
- [x] Smoke tests — billing and router integration verified

## Phase 2 — Integration and Docs

Connect the ecosystem, ship the cloud dashboard, and enable end-to-end workflows.

- [ ] docs.talocode.site — comprehensive documentation site
- [ ] dashboard.talocode.site — web dashboard for wallet, usage, and API keys
- [ ] Codra — Talocode Cloud integration for skill execution
- [ ] Tera — Browser context capture API with Talocode Cloud billing
- [ ] ClipLoop — hosted video rendering API
- [ ] Stripe top-up — live Stripe integration for wallet funding

## Phase 3 — Platform Scale

Expand the hosted platform with more APIs, SDKs, and ecosystem integrations.

- [ ] WorkLane — hosted workflow runs with approval UI
- [ ] SignalLane — business signal API
- [ ] Tradia — trading intelligence API
- [ ] SDKs — TypeScript, Python, and REST SDKs for all product APIs
- [ ] MCP integrations — Model Context Protocol servers for every product
- [ ] Provider marketplace — bring-your-own AI provider for the router

## How to Influence the Roadmap

Open an issue in this repository or any product repository. Talocode is community-driven — the roadmap evolves based on what builders actually need.
